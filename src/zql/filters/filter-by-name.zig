...
pub fn filterByName(allocator: std.mem.Allocator, result: *std.ArrayList([]const u8), playlist: CmupPlaylist, filter: Filter) !void {
        ...
        switch (filter.match_type.data) {
            .Contains => {
                if (std.ascii.indexOfIgnoreCase(name, target) != null) {
                    try result.append(allocator, track);
                }
            },
            .Is => {
